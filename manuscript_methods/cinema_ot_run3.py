...
#usage: ./cinema_ot_run.py sample_list_file data_name data_root_dir var_name save_root_dir perturb_only expand_only
...
def cinema_process(pat, perturb_only, expand_only):
    ...
    if perturb_only == "yes":
        cf, ot, de = co.cinemaot.cinemaot_unweighted(adata,obs_label='expand', ref_label='yes', expr_label='no',mode='parametric',thres=0.5,smoothness=1e-5,eps=1e-3)
    else:
    ...
    if perturb_only == "yes":
        adata.obsm['cf'][adata.obs['expand']=='yes',:] = np.matmul(ot/np.sum(ot,axis=1)[:,None],cf[adata.obs['expand']=='no',:])
    else:
    ...
    if de.obs["expand"].nunique() == 1:
        return "apple"

    k = 5

    if perturb_only == "yes":
        latent_basal_entropy  = entropy_process(adata, "cf", 'expand')
        expand_entropy = entropy_process(de, "X_embedding","expand")
        if expand_only == "yes":
            de1 = de[de.obs["expand"] == "yes"]
        else:
            de1 = de.copy()
        ...
        expand_entropy = entropy_process(de, "X_embedding","expand")
        if expand_only == "yes":
            de1 = de[(de.obs["expand"].values == "yes") & (de.obs["condition"].values == "perturb")]
        else:
            de1 = de[de.obs["condition"] == "perturb"]
    
    if de1.n_obs < 50:
        corr_res = ["NA", "NA"]
    else:
        corr_res = clonality_correlation(de1, k)

    results = [latent_basal_entropy.mean(), expand_entropy.mean(), corr_res[0], corr_res[1]]
    return(results)
...
for pat in pat_list:
    print(pat)
    pat_res = cinema_process(pat, perturb_only, expand_only)
    if pat_res == "apple":
        continue
    #sinkhorn_knopp does not fit
    
    df = pd.DataFrame(pat_res)
    df.index = ["latent_basal_entropy","expand_entropy", "expand_corr", "corr_pval"]
    total_result[pat] = pd.Series(pat_res)
total_result.index = ["latent_basal_entropy","expand_entropy", "expand_corr", "corr_pval"]
