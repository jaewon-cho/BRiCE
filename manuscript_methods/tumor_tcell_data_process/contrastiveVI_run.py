...
# In[1]:
#usage: ./contrastiveVI_run.py sample_list_file data_name data_root_dir var_name save_root_dir
#sample_file_list:
#data_name: gse236581
...
def contrastive_process(pat):
    ...
    ContrastiveVI.setup_anndata(adata)
    
    target_adata = adata[adata.obs["Tissue"] != "Normal"].copy()
    background_adata = adata[adata.obs["Tissue"] == "Normal"].copy()
    ...
    background_indices = np.where(adata.obs["Tissue"] == "Normal")[0]
    target_indices = np.where(adata.obs["Tissue"] != "Normal")[0]
    ...
    latent_basal_entropy  = entropy_process(adata, "background_rep", 'Tissue')
    ...
    random_result = []
    sc.pp.neighbors(target_adata, use_rep= "salient_rep", n_neighbors=30, knn = True)
    for i in range(1000):
        #print(i)
        random_exp_entropy = entropy_rand_process(target_adata, "expand")
    ...
    results = [latent_basal_entropy.mean(), latent_after_entropy.mean(), p_value, expand_entropy.mean(), random_mean, expand_rank/1000]
    return(results)
...
for pat in pat_list:
    ...
    df.index = ["latent_basal_entropy","latent_after_entropy","p_value","expand_entropy","random_entropy","exp_pval"]
    total_result[pat] = pd.Series(pat_res)
total_result.index = ["latent_basal_entropy","latent_after_entropy","p_value","expand_entropy","random_entropy","exp_pval"]
