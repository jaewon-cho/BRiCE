...
#var_name: tumor_deg, expand_deg, corr 
...
for pat in pat_list:
    ...
    if var_name == "tumor_deg":
        sc.tl.rank_genes_groups(adata1, groupby='Tissue', method='wilcoxon')
        deg_df = sc.get.rank_genes_groups_df(adata1, group='Tumor')
        sig_genes =  deg_df.loc[deg_df['pvals_adj'] < 0.05, 'names'].tolist()
        new_var_gn = set(sig_genes)
    elif var_name == "expand_deg":
        adata2 = adata1[adata1.obs['Tissue'] == 'Tumor'].copy()
