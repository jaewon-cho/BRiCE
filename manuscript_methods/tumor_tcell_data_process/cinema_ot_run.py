...
def cinema_process(pat):
    ...
    cf, ot, de = co.cinemaot.cinemaot_unweighted(adata,obs_label='Tissue', ref_label='Tumor', expr_label='Normal',mode='parametric',thres=0.5,smoothness=1e-5,eps=1e-3)
    if True in np.isnan(ot):
        return "apple"
    
    adata.obsm['cf'] = cf.copy()
    adata.obsm['cf'][adata.obs['Tissue']=='Tumor',:] = np.matmul(ot/np.sum(ot,axis=1)[:,None],cf[adata.obs['Tissue']=='Normal',:])
    sc.pp.pca(de)
    ...
    latent_basal_entropy  = entropy_process(adata, "cf", 'Tissue')
