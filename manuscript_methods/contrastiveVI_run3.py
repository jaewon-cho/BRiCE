...
def clonality_correlation(adata1, k):
    sc.pp.neighbors(adata1, use_rep = 'salient_rep', n_neighbors=k, knn = True)
...
def contrastive_process(pat):
    ...
    if perturb_only == "yes":
        ...
        if expand_only == "yes":
            target_adata1 = target_adata[(target_adata.obs["expand"] == "yes") & (target_adata.obs["condition"] == "perturb")]
        else:
